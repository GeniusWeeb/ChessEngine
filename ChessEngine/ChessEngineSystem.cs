...
namespace ChessEngine
{
    public class ChessEngineSystem : IDisposable
    {
        ...
        public async Task CheckForGameModeAndPerform()
        {
            ...
            switch (GameStateManager.Instance.GetCurrentGameMode)
            {
                ...
                case GameMode.PlayerVsBot:
                    if (GameStateManager.Instance.player1MoveCol ==
                        GameStateManager.Instance.playerToMove) // for the first turn
                    {
                ...
                case GameMode.BotVsBot:
                    ...
                    else if (GameStateManager.Instance.player2MoveCol ==
                             GameStateManager.Instance.playerToMove) // for the first turn
                    {
        ...
        private void BotMove(ref BotBrain brain)
        {
            ...
            Console.WriteLine("--------------------------------------------------------------------------------------");
            Console.WriteLine(
                $"Bot {GameStateManager.Instance.playerToMove.ToString()}  is  gonna make the move , Waiting ...");
            Console.ResetColor();
        ...
        private void PassDataToBoard(string data)
        {
            ...
            Protocols finalData = new Protocols(ProtocolTypes.VALIDATE.ToString(), validationData,
                GameStateManager.Instance.GetTurnToMove.ToString());
        ...
        public void UpdateUIWithNewIndex(int oldIndex, int newIndex)
        {
            ...
            Protocols finalData = new Protocols(ProtocolTypes.UPDATEUI.ToString(), data,
                GameStateManager.Instance.playerToMove.ToString());
            SendDataToUI(finalData);
        }
